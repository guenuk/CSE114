...
public class Pig {
	...
	String [] splitString = sent.split("[ \n\t]+");
	String sentence = "";
	for(int i=0; i<splitString.length; i++){
	    sentence += pigifyWord(splitString[i]) + " ";
	}
	return sentence;
    }
	...
	else if (firstVowel(word) >= 1){
	    return (word.substring(firstVowel(word))).concat(word.substring(0,firstVowel(word))) + "ay";
	}
	else{
	    return word;
	}
